...
async function sendFile(filePath: string, options: Partial<JaiStaticOptions>, req: IncomingMessage, res: ServerResponse, cb?: (error?: Error) => void, showError: boolean = true): Promise<boolean> {
  try {
    ...
    if (fileStat.isDirectory()) {
      ...
      return sendFile(indexPath, mergedOptions, req, res, cb, showError);
    }
    ...
    console.error(error);
    return sendErrorResponse(error as Error, res, cb);
  }
}
...
async function createStatic(options: JaiStaticOptions, req: IncomingMessage, res: ServerResponse, next: NextFunction): Promise<void> {
  ...
  const sent = await sendFile(filePath, mergedOptions, req, res, undefined, false);
  if (sent) return;
  return next();
}
